...
* GoのDDD実装における`main.go`の肥大化問題を、DIライブラリ`uber-go/fx`（以下、fx）で解決します。
...
このアプローチには、運用規模が大きくなるにつれて以下の課題が発生します。特に「依存を1つ追加しただけで初期化順序とテスト差し替えの両方を見直す」点が壊れポイントになりがちです。

* **依存順序の管理**: 「Aを作るにはBが必要だから、先にBを作らないと...」という依存関係のパズルを手動で解き続ける必要があります。依存を1つ追加するだけで初期化順序の並べ替えが発生します。
* **共通コンポーネントの引き回し**: Logger、Config、Tracerなど、アプリ全体で使うコンポーネントを末端のRepositoryまで届けるために、中間層のすべてのコンストラクタを経由してバケツリレーする必要があります。テストで差し替える際も同じ経路を通る必要があります。
* **修正コスト**: あるServiceの依存が1つ増えただけで、main.go の初期化フロー全体を見直す必要が出てきます。結果として main.go が「壊れポイント」になりがちです。

### uber-fx を導入するとどうなるか

fx を導入すると、上記の手続き的なコードは、「何を使うか（依存の定義）」 を宣言するだけのスタイルに変わります。後段で詳述する `fx.Invoke` を使うと、起動時に必ず呼びたい処理（例: サーバー起動）も宣言的に登録できます。
...
`fx.Provide` はコンストラクタの引数の型から必要なインスタンスを自動で解決し、注入してくれます。依存関係のグラフをライブラリが構築するため、手動で初期化順序を管理する必要がありません。起動時の処理（`fx.Invoke` で登録する関数）は後述します。
...
* モジュールの階層イメージ  
    * `presentation.Module`（最上位）  
        * `application.Module`  
            * `infrastructure.Module`

上記のように「上位が下位を内包する」構造にすると、依存関係と組み立て順序をコードで明示でき、`main.go` の壊れやすさを抑えられます。

## 6. 実践②：fx.Annotateでインターフェース注入

実践①のように、単純に `fx.Provide` を使うだけでは、DDD の依存関係の原則に問題が生じます。ここでのポイントは「構造体を返すコンストラクタのまま、登録だけインターフェースとして扱う」ことです。
...
具体的には、Application 層から `db.NewDatabase` の返り値を引数に取るコンポーネントを `fx.Provide` に登録しても、依存値が見つからずアプリケーション起動時にエラーが報告されます。

## 8. 実践④：fx.Supplyとタグで設定値を注入
...
これでは新しい API エンドポイントを追加するたびに、本来修正する必要のない `ServiceServer` のコードに手を入れることになり、開放閉鎖の原則（拡張に対しては開いていて、修正に対しては閉じているべき）に違反します。
...
`RouteRegistrar` は「自分のエンドポイントをルーターに登録する」ための共通フックとなるインターフェースです。
...
* OnStart: 非ブロッキングでサーバーを立ち上げる
* OnStop: 受け取ったシグナルをトリガーにGraceful Shutdownを走らせる
...
例えば、テスト時はロガーを無効にするために、`fx.Replace` を使って既存の `Logger` 定義を上書きすることができます。外部APIクライアントやメッセージキューのPublisherも同じように差し替えられます。
...
1. mainの肥大化に対して: モジュールで階層を束ね、依存初期化を宣言化。
2. 依存逆転・カプセル化に対して: fx.Annotate / fx.Private でインターフェース注入と漏れ防止を強制。
3. 拡張性に対して: Group機能とタグで「追加は1行、修正なし」を実現。
4. 運用面に対して: fx.Lifecycle でGraceful Shutdownを標準化。
