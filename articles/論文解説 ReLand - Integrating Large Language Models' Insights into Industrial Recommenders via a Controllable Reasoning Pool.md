---
title: "論文解説：ReLand - LLMの推論結果を「使い回す」ことで産業用推薦システムに統合するAlipayの手法"
emoji: "🚀"
type: "tech"
...
### 論文が言及している限界と今後の展望 (Limitations)

著者らは、ReLandの有効性を証明する一方で、今後の研究に向けた以下の3つの限界を挙げています。

1. **推薦理由の多様性が少ない**: 計算効率を優先して類似ユーザーの推論結果を使い回すため、システム全体でユーザーに割り当てられる推薦理由が似通ってしまう傾向があります。今後は、各ユーザーに複数の推論を検索・統合して多様性を高めるアプローチが求められます。
2. **最適なシードユーザー数の決定**: シードユーザーの数を増やしても、推薦性能が常に線形に向上するわけではありません。実務では計算リソースとのトレードオフで調整していますが、ROI（投資対効果）を最大化する最適なシードユーザー数を効率的に決定する手法の探索が課題です。
3. **新規ユーザーのコールドスタート問題**: 過去の行動履歴をコンテキストとしてLLMに推論させる仕組み上、履歴が全くない新規ユーザーには機能しにくいです。今後は、年齢や職業などのユーザープロファイルを利用したり、新規ユーザーに特定のシードユーザーを割り当てたりするなどの改善が必要となります。
