...
**前提バージョン**: 第4節で紹介する `errors.Join` は Go 1.20 以降で使えます。それ以前のバージョン向けには、第3節で `fmt.Errorf` による代替パターンも示します。
...
## 2. ローカル変数 err を defer で更新しても戻り値に反映されない
...
## 4. errors.Join で主処理と後処理のエラーを合成する（Go 1.20+）

前節の `if` 分岐を `errors.Join` で置き換えると、次のように書けます。
...
注意点もあります。`errors.Join` が返すエラーのメッセージは、デフォルトでは改行区切りになります。
...
|後処理エラーの性質|呼び出し元の対処可能性|推奨|
|---|---|---|
|データ整合性に影響する（flush, rename 等）|対処できる|名前付き戻り値 + `errors.Join`|
|観測・調査に必要だが主処理結果に影響しない|対処不要|`defer` 内でログ出力|
|エラーを返さない、または失敗が実質的に無害|対処不要|`defer someFunc()` のまま|

### 後処理エラーを返すべき例

典型的なのは、ファイル書き込み後の `Close()` です。`os.File` の `Close()` は、`Write()` の時点ではまだカーネル側でバッファされていた書き込みのエラー（NFS や一部ストレージで遅延報告されるもの）を、close 時にまとめて返してくることがあります。そのため、`Close()` の失敗が書き込み失敗を意味することがあります。
...
func f() (err error) {
	...
	// この err は名前付き戻り値とは別の変数なので、defer からは参照されない
	if result, err := doSomething(); err != nil {
		_ = result
		return err // ← 名前付き戻り値 err ではなく、ここで宣言された err を返す
	}
...
`if` ブロック内で `:=` を使うと、新しいスコープで別の `err` 変数が宣言されます。この `err` は名前付き戻り値の `err` とは別の変数なので、`defer` が参照する `err` はこの `if` ブロック内の変更を知りません。このようなシャドーイングは、`go vet` の `shadow` チェッカーで検出できるので、CI に組み込んでおくと安心です。
